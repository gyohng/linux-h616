...
static void dump_reg(struct sunxi_i2c *i2c, u32 offset, u32 len)
{
	u32 i;
	u8 buf[64], cnt = 0;
...
static int sunxi_i2c_probe(struct platform_device *pdev)
{
	...
	i2c->rstc = devm_reset_control_get_optional_exclusive(&pdev->dev, NULL);
	if (IS_ERR(i2c->rstc)) {
		ret = PTR_ERR(i2c->rstc);
		goto erstc;
	}
